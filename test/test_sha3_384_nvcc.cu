...
using digest_type = typename boost::crypt::sha3_384_hasher::return_type;
...
__global__ void cuda_test(char** in, digest_type* out, int numElements)
{
    ...
    if (i < numElements)
    {
        ...
        out[i] = boost::crypt::sha3_384(in_span).value();
    }
}
...
int main()
{
    try
    {
        ...
        for(int i = 0; i < numElements; ++i)
        {
            ...
            results.emplace_back(boost::crypt::sha3_384(in).value());
        }
        double t = w.elapsed();
