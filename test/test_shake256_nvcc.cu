...
using digest_type = typename boost::crypt::shake256_hasher::return_type;
...
__global__ void cuda_test(char** in, digest_type* out, int numElements)
{
    ...
    if (i < numElements)
    {
        ...
        out[i] = boost::crypt::shake256(in_span).value();
    }
}
...
int main()
{
    try
    {
        ...
        for(int i = 0; i < numElements; ++i)
        {
            ...
            results.emplace_back(boost::crypt::shake256(in).value());
        }
        double t = w.elapsed();
