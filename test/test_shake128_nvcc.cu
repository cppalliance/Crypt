...
using digest_type = typename boost::crypt::shake128_hasher::return_type;
...
__global__ void cuda_test(char** in, digest_type* out, int numElements)
{
    ...
    if (i < numElements)
    {
        ...
        out[i] = boost::crypt::shake128(in_span).value();
    }
}
...
int main()
{
    try
    {
        ...
        for(int i = 0; i < numElements; ++i)
        {
            ...
            results.emplace_back(boost::crypt::shake128(in).value());
        }
        double t = w.elapsed();
