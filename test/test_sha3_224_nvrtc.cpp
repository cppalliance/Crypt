...
using digest_type = boost::crypt::sha3_224_hasher::return_type;
extern "C" __global__
void test_sha3_224_kernel(char** in, digest_type* out, int numElements)
{
...
int main()
{
    try
    {
        ...
        res = nvrtcCreateProgram(&prog, cuda_kernel, "test_sha3_224_kernel.cu", 0, nullptr, nullptr);
        checkNVRTCError(res, "Failed to create NVRTC program");

        nvrtcAddNameExpression(prog, "test_sha3_224_kernel");
        ...
        checkCUError(cuModuleGetFunction(&kernel, module, "test_sha3_224_kernel"), "Failed to get kernel function");
        ...
        for (int i = 0; i < numElements; ++i)
        {
            auto res = boost::crypt::sha3_224(input_vector1[i]);
