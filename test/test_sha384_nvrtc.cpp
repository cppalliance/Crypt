...
using digest_type = boost::crypt::sha384_hasher::return_type;
...
using digest_type = boost::crypt::sha384_hasher::return_type;
extern "C" __global__
void test_sha384_kernel(char** in, digest_type* out, int numElements)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if (i < numElements)
    {
        out[i] = boost::crypt::sha384(in[i]);
    }
}
...
int main()
{
    try
    {
        ...
        res = nvrtcCreateProgram(&prog, cuda_kernel, "test_sha384_kernel.cu", 0, nullptr, nullptr);
        checkNVRTCError(res, "Failed to create NVRTC program");

        nvrtcAddNameExpression(prog, "test_sha384_kernel");
        ...
        checkCUError(cuModuleGetFunction(&kernel, module, "test_sha384_kernel"), "Failed to get kernel function");
        ...
        for (int i = 0; i < numElements; ++i)
        {
            auto res = boost::crypt::sha384(input_vector1[i]);
